...
var AutoFilename = class extends import_obsidian.Plugin {
  ...
  async renameFile(file, noDelay = false) {
    ...
    if (!isUntitledPattern.test(fileBaseName))
      return;
    if (noDelay === false) {
      if (onTimeout) {
    ...
    if (this.settings.useHeader && content[0] === "#") {
      ...
      for (let i = 0; i < headerArr.length; i++) {
        if (content.startsWith(headerArr[i])) {
          ...
          if (index !== -1)
            content = content.slice(headerArr[i].length, index);
          break;
        }
      }
    }
  ...
  async renameFileWithText(file, selectedText) {
    const illegalChars = '\\/:*?"<>|#^[]';
    const illegalNames = [
      "CON",
      "PRN",
    ...
    const isIllegalName = newFileName === "" || illegalNames.includes(newFileName.toUpperCase());
    if (isIllegalName) {
      new import_obsidian.Notice("Invalid filename. Please select valid text.");
      return;
    }
    const parentPath = !file.parent || file.parent.path === "/" ? "" : file.parent.path + "/";
    let newPath = `${parentPath}${newFileName}.md`;
    if (file.path === newPath) {
      new import_obsidian.Notice("Filename is already the same.");
      return;
    }
    let counter = 1;
    let fileExists = this.app.vault.getAbstractFileByPath(newPath) !== null;
    ...
    await this.app.fileManager.renameFile(file, newPath);
    new import_obsidian.Notice(`File renamed to: ${newFileName}${counter > 1 ? ` (${counter})` : ""}`);
  }
  async saveSettings() {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (selection && selection.trim().length > 0) {
          menu.addItem((item) => {
            item.setTitle("\u4EE5\u9009\u53D6\u6587\u672C\u91CD\u547D\u540D").setIcon("pencil").onClick(async () => {
    ...
    this.registerEvent(
      this.app.workspace.on("file-open", (file) => {
        ...
        if (!document.body.classList.contains("show-inline-title"))
          return;
        const shouldHide = this.settings.isTitleHidden && inTargetFolder(file, this.settings);
...
var AutoFilenameSettings = class extends import_obsidian.PluginSettingTab {
  display() {
    ...
    new import_obsidian.Setting(this.containerEl).setName("Include Emojis").setDesc("Include Emojis in the filename.").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.includeEmojis).onChange(async (value) => {
